    ...
    <div className="tab-view">
      ...
      {error && !health && (
        <div style={{
          ...
          Unable to reach health endpoint: {error}
        </div>
      )}
      ...
      {/* Service Cards — always render, even without health data */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '14px', marginBottom: '28px' }}>
        {SERVICES.map((svc) => {
          const data = health?.services?.[svc.key] || null;
          const status = data?.status || (health ? 'disconnected' : 'unknown');
      ...
      {health && Object.values(latencyHistory).some((h) => h.length > 0) && (
        <div>
          <h3 style={{ fontSize: '16px', fontWeight: '700', marginBottom: '14px', color: 'var(--text)' }}>
